...
class PortScanner:
    ...
    def scan_ports(self, ip):
        ...
        responses, _ = sr(tcp_packets + udp_packets, timeout=1, verbose=0)
        workstations = dict()
        ip_service_detected = dict()
        for response in responses:
            sent_packet, received_packet = response

            # Check if we sent a TCP packet
            ...
            # Check if we sent a UDP packet
            elif sent_packet.haslayer(UDP):
                if not received_packet.haslayer(ICMP):  # ICMP layer means port is unreachable
    ...
    def start_scan(self):
        # Use a semaphore to limit number of threads
